/**
 * oxapayService.js — OxaPay API client.
 *
 * Handles invoice creation, status checks, white-label payments,
 * slave account management, and HMAC verification.
 */

'use strict';

const axios  = require('axios');
const crypto = require('crypto');
const config = require('../config');

const OXAPAY_BASE  = 'https://api.oxapay.com';
const MERCHANT_KEY = config.oxapay.merchantKey;
const SANDBOX      = config.oxapay.sandbox;

if (!MERCHANT_KEY) {
  console.warn('[oxapay] OXAPAY_MERCHANT_KEY not set — payment routes will fail');
}

const oxaClient = axios.create({
  baseURL: OXAPAY_BASE,
  timeout: 10_000,
  headers: {
    merchant_api_key: MERCHANT_KEY,
    'Content-Type':   'application/json',
  },
});

function unwrap(data) {
  return (data && typeof data.data === 'object' && data.data !== null)
    ? data.data
    : data;
}

// ── Helpers ──────────────────────────────────────────────────────

/**
 * Timing-safe HMAC-SHA512 comparison.
 * Reused by both merchant and slave callback verification.
 */
function verifyHmac(rawBody, receivedHmac, key) {
  if (!key || !rawBody || !receivedHmac) return false;
  const expected = crypto
    .createHmac('sha512', key)
    .update(rawBody)
    .digest('hex');
  try {
    return crypto.timingSafeEqual(
      Buffer.from(expected,     'hex'),
      Buffer.from(receivedHmac, 'hex'),
    );
  } catch {
    return false;
  }
}

// ── Invoice ──────────────────────────────────────────────────────

const createInvoice = async ({ amount, currency, orderId, callbackUrl, returnUrl, email, description }) => {
  const body = {
    amount,
    currency,
    lifetime:          60,
    fee_paid_by_payer: 0,
    callback_url:      callbackUrl,
    return_url:        returnUrl,
    order_id:          orderId,
    description:       description || `AvgExchange deposit — ${currency}`,
    sandbox:           SANDBOX,
  };
  if (email) body.email = email;

  const { data } = await oxaClient.post('/v1/payment/invoice', body);
  console.log('[oxapay] createInvoice response:', JSON.stringify(data));

  if (data.status !== 200) {
    throw new Error(`OxaPay invoice error: ${data.message || JSON.stringify(data)}`);
  }

  const payload = unwrap(data);
  const trackId = payload.track_id ?? payload.trackId;
  const payLink = payload.payment_url ?? payload.payLink ?? payload.pay_link;

  if (!trackId) throw new Error(`OxaPay returned no track_id. Response: ${JSON.stringify(data)}`);
  if (!payLink) throw new Error(`OxaPay returned no payment_url. Response: ${JSON.stringify(data)}`);

  return { trackId, payLink };
};

// ── Status ───────────────────────────────────────────────────────

const getPaymentStatus = async (trackId) => {
  const { data } = await oxaClient.get(`/v1/payment/${trackId}`);
  if (data.status !== 200) {
    throw new Error(`OxaPay status error: ${data.message || JSON.stringify(data)}`);
  }
  const raw = unwrap(data).status ?? '';
  return raw.charAt(0).toUpperCase() + raw.slice(1);
};

const getWhiteLabelStatus = async (trackId) => {
  const { data } = await axios.post(
    `${OXAPAY_BASE}/merchants/inquiry`,
    { merchant: MERCHANT_KEY, trackId },
    { timeout: 10_000, headers: { 'Content-Type': 'application/json' } }
  );
  if (data.result !== 100) {
    throw new Error(`OxaPay inquiry error: ${data.message || JSON.stringify(data)}`);
  }
  const raw = data.status ?? '';
  return raw.charAt(0).toUpperCase() + raw.slice(1);
};

// ── HMAC ─────────────────────────────────────────────────────────

const verifyCallbackHmac = (rawBody, receivedHmac) => verifyHmac(rawBody, receivedHmac, MERCHANT_KEY);
const verifySlaveHmac = (rawBody, receivedHmac, slaveKey) => verifyHmac(rawBody, receivedHmac, slaveKey);

// ── White-label ──────────────────────────────────────────────────

const createWhiteLabelPayment = async ({
  amount, currency, payCurrency, network,
  orderId, callbackUrl, email, description,
  lifeTime = 1440,
  underPaidCover = 5,
  feePaidByPayer = 0,
}) => {
  const body = {
    merchant:       MERCHANT_KEY,
    amount,
    payCurrency,
    callbackUrl,
    lifeTime,
    feePaidByPayer,
    underPaidCover,
  };
  if (currency)    body.currency    = currency;
  if (network)     body.network     = network;
  if (orderId)     body.orderId     = orderId;
  if (email)       body.email       = email;
  if (description) body.description = description;

  const { data } = await axios.post(`${OXAPAY_BASE}/merchants/request/whitelabel`, body);

  if (data.result !== 100) {
    throw new Error(`OxaPay white-label error: ${data.message || JSON.stringify(data)}`);
  }

  return {
    trackId:     String(data.trackId),
    address:     data.address,
    memo:        data.memo || '',
    payAmount:   data.payAmount,
    payCurrency: data.payCurrency,
    network:     data.network,
    qrCode:      data.QRCode,
    expiredAt:   data.expiredAt,
    rate:        data.rate,
  };
};

// ── Slave accounts ───────────────────────────────────────────────

const createSlaveAccount = async (name, webhookUrl) => {
  const { data } = await oxaClient.post('/v1/merchants/add', { name, webhook: webhookUrl });

  if (data.status !== 200) {
    throw new Error(`OxaPay createSlave error: ${data.message || JSON.stringify(data)}`);
  }

  const payload  = unwrap(data);
  const slaveKey = payload.apiKey ?? payload.api_key;
  if (!slaveKey) {
    throw new Error(`OxaPay returned no slave apiKey. Response: ${JSON.stringify(data)}`);
  }
  return slaveKey;
};

const getWalletAddress = async (slaveKey, currency, network) => {
  const { data } = await axios.post(
    `${OXAPAY_BASE}/v1/wallets`,
    { currency, network },
    { timeout: 10_000, headers: { merchant_api_key: slaveKey, 'Content-Type': 'application/json' } }
  );

  if (data.status !== 200) {
    throw new Error(`OxaPay getWalletAddress error: ${data.message || JSON.stringify(data)}`);
  }

  const address = unwrap(data).address;
  if (!address) {
    throw new Error(`OxaPay returned no address. Response: ${JSON.stringify(data)}`);
  }
  return address;
};

module.exports = {
  createInvoice,
  createWhiteLabelPayment,
  getPaymentStatus,
  getWhiteLabelStatus,
  verifyCallbackHmac,
  verifySlaveHmac,
  createSlaveAccount,
  getWalletAddress,
};
